...
class DSSM(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.model_path = os.path.join(
            self.model_name_or_path, "pytorch_model.bin"
        )
        self.vocabulary_path = os.path.join(
            self.model_name_or_path, "vocabulary.bin"
        )
        self.config_path = os.path.join(self.model_name_or_path, "config.json")
        self.load()

    def make_perceptron_layer(
        self, in_f, out_f, activation=None, dropout=None, batch_norm=False
    ):
    ...
    def make_mlp_layer(
        self, in_features, mlp_num_layers, mlp_num_units, mlp_num_fan_out
    ):
        mlp_sizes = [
            in_features,
        ...
        if self.mlp_activation_func in activation_dict:
            ...
            raise ValueError(
                ...
                + str(self.mlp_activation_func)
            )
        mlp = [
            self.make_perceptron_layer(
                in_f, out_f, activation=activation, batch_norm=False
            )
    ...
    def build(self):
        ...
        self.mlp_left = self.make_mlp_layer(
            self.in_features,
            self.mlp_num_layers,
            self.mlp_num_units,
            self.mlp_num_fan_out,
        )
        self.mlp_right = self.make_mlp_layer(
            self.in_features,
            self.mlp_num_layers,
            self.mlp_num_units,
            self.mlp_num_fan_out,
    ...
    def criterion(self, softmax_qp):
        ...
        # When used for classifiers the log-softmax has the effect of
        #   heavily penalizing the model when it fails to
        #   predict a correct class.
        #   Whether or not that penalization works well for
        #   solving your problem is open to your testing,
        #   so both log-softmax and softmax are worth using.
    ...
    def forward(self, q, p, ns, norm=False):
        ...
        # ns.shape = [batch_size, n_negsample, n_vocab]
        # ns.permute(1, 0, 2).shape = [n_negsample, batch_size, n_vocab]
        #   So, each n has the shape of [batch_size, n_vocab]
        #   and self.mlp_right(n).shape = [batch_size, 128]
        #   out_ns = encoded negative samples [ns[0], ns[1], ns[2], ns[3]]
        #       , where each ns[i].shape = [batch_size, 128]
        out_ns = [self.mlp_right(n) for n in ns.permute(1, 0, 2)]

        # Relevance measured by cosine similarity
        if not self.gamma:
            cos_qp = self.learn_gamma(
                F.cosine_similarity(out_q, out_p, dim=1).unsqueeze(1)
            )
            cos_qns = [
                self.learn_gamma(
                    F.cosine_similarity(out_q, out_n, dim=1).unsqueeze(1)
                )
                for out_n in out_ns
            ]
        else:
    ...
    def fit(
        ...
        for q, d in zip(queries, documents):
            rand = [
                documents[i] for i in np.random.randint(total, size=num_neg)
            ]
            query.append(q)
            doc_p.append(d)
            doc_ns.append(rand)
        ...
        for epoch in range(epochs):
            ...
            for q, p, ns in train_iterator:
                ...
                #   because PyTorch accumulates the gradients
                ...
                logging.info(
                    r"epoch: %s/%s, batch:%s/%s, lr: %s, loss: %s"
            ...
            logging.info(
                r"epoch: %s/%s, lr: %s, train loss: %s, eval loss: %s"
                % (epoch + 1, epochs, lr, train_loss, eval_loss)
            )
